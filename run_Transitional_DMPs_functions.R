...
# Multinomial Cochran-Armitage Test
...
# setwd
setwd("/media/Data/WGBS/LOAD_MCI/Inputs/CONTROL_MCI_LOAD_New/")
infiles <- list.files(pattern="RData$")

meth.mat <- c()
for (i in 1:length(infiles)) {

	cat(paste0("Working on ",infiles[i],"\n"))
	load(infiles[i])

	meth.mat.sub <- bsseq::getMeth(bs, type = "raw")
	rownames(meth.mat.sub) <- paste0(seqnames(bs), ":", start(bs))
	meth.mat <- rbind(meth.mat, meth.mat.sub)
...
meth.mat.sig <- meth.mat[which(df$lfdr.from.ss<0.05 & ((df$pi.diff.mci.ctrl > 0.0125 & df$pi.diff.load.mci > 0.0125) | (df$pi.diff.mci.ctrl < -0.0125 & df$pi.diff.load.mci < -0.0125))),]

meth.mat.sig <- t(meth.mat.sig)

cochranArmitage_pvalues <- c()

for (iii in 1:ncol(meth.mat.sig)) { 
	cpg_to_test <- meth.mat.sig[,iii]
	caResults <- multiCA.test(targets$diagnostic_group ~ meth.mat.sig[,iii])
	p <- as.data.frame(caResults$overall$p.value)
	colnames(p) <- "pvalue"
	cochranArmitage_pvalues <- rbind(cochranArmitage_pvalues, p)
}
save(meth.mat.sig,sig,cochranArmitage_pvalues,file="/media/Data/WGBS/LOAD_MCI/Results/CONTROL_MCI_LOAD/cochran_armitage_results/caResults.rdata")
